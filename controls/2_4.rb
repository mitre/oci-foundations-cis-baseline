control '2_4' do
  ...
  desc "Network security groups provide stateful filtering of ingress/egress network traffic to OCI resources. It is recommended that no security group allows unrestricted ingress access to port 3389.
...
Removing unfettered connectivity to remote console services, such as Remote Desktop Protocol (RDP), reduces a server's exposure to risk."
  desc 'check', %q(From CLI: Issue the following command, it should not return anything. for region in $(oci iam region-subscription list | jq -r '.data[] | ."region-name"')
    do
