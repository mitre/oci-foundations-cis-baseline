...
Encryption of block volumes provides an additional level of security for your data. Management of encryption keys is critical to protecting and accessing protected data. Customers should identify block volumes encrypted with Oracle service managed keys in order to determine if they want to manage the keys for certain volumes and then apply their own key lifecycle management to the selected block volumes.'
  desc 'check', %q(From Console: Login to the OCI Console. Click the search bar at the top of the screen. Type 'Advanced Resource Query' and press return. Click Advanced resource query . Enter the following query in the query box: query volume resources For each block volume returned, click the link under Display name . Ensure the value for Encryption Key is not Oracle-managed key . Repeat for other subscribed regions. From CLI: Execute the following command: for region in $(oci iam region-subscription list --all| jq -r '.data[] | ."region-name"')
do
  ...
  done Ensure the query returns no results.)
  desc 'fix', 'From Console: Follow the audit procedure above. For each block volume returned, click the link under Display name. If the value for Encryption Key is Oracle-managed key , click Assign next to Oracle-managed key . Select a Vault Compartment and Vault . Select a Master Encryption Key Compartment and Master Encryption key . Click Assign . From CLI: Follow the audit procedure. For each boot volume identified, get the OCID. Execute the following command: oci bv volume-kms-key update –volume-id <volume OCID> --kms-key-id <kms key OCID>'
